/*
 * Original work copyright (c) 2013 Andrea Arteaga
 * All rights reserved.
 *
 * This file is a modified version of the original source code by
 * Andrea Arteaga. The modifications in this version add OpenACC support
 * to the BITREP routines.
 *
...
double __internal_old_exp_kernel(double x, int scale) {
  ...
  if (std::abs(i) < 1021) {
    ...
    zz.i[0] = 0;
    zz.i[1] = j; /* 2^-54 if a is denormal, 2.0 otherwise */
    z = zz.d;
    t = __BITREPFMA(t, z, z);

    zz.i[0] = 0;
    zz.i[1] = k; /* 2^-54 if a is denormal, 2.0 otherwise */
    z = zz.d;
    z = t * z;
  }
  return z;
